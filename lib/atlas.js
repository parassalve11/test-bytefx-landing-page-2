...
export function getAtlasReply(message) {
  ...
  if (/\b(mt4|metatrader ?4)\b/.test(text)) {
    return { text: 'ByteFX does not offer MetaTrader 4. You can trade on MetaTrader 5 or TradingView instead — both are in our platform guide.', links: [{ label: 'Explore platforms', href: '#platform-guide' }] };
  }
  if (/\b(mt5|metatrader|tradingview|trading view|platform|platforms)\b/.test(text)) {
    return { text: 'You can explore MetaTrader 5 and TradingView in our platform guide. Select a platform to see its tools and official website. For availability with your ByteFX account, check with support.', links: [{ label: 'Explore platforms', href: '#platform-guide' }, { label: 'Check account compatibility', href: `mailto:${site.email}` }] };
  }
  ...
  if (/\b(markets|market|forex|instruments|crypto|commodities|indices|shares|gold)\b/.test(text)) {
    return { text: 'The markets section covers Forex, Indices, Crypto, Commodities and Shares. Check the app or support team for the instruments available to your account.', links: [{ label: 'Explore markets', href: '#markets' }] };
  }
